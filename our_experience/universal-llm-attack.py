...
# gradient hook 是在反向传播过程中用于捕获梯度信息的一种机制
def extract_grad_hook(module, grad_in, grad_out):
    extracted_grads.append(grad_out[0])

# 为给定的语言模型（或神经网络模型）中的特定嵌入层添加梯度钩子
...
# 将触发词转换为 Torch Tensor,并将其重复 batch_size 次以匹配目标文本的数量
...
def get_loss(language_model, batch_size, trigger, target, device='cuda'):
    ...
    # 创建一个与 tensor_trigger 形状相同的 mask_out,其所有元素均为 -1
    # 我们将在损失计算时将触发词的损失置零,即不计算触发词的损失
    mask_out = -1 * torch.ones_like(tensor_trigger) 

    # 将触发词和目标文本合并成一个新的 Torch Tensor
    # 这样合并后的文本就包含了触发词作为上下文
    ...
    loss = language_model(lm_input, labels=mask_and_target).loss

    return loss

# hotflip_attack 函数的主体部分
